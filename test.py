...
dict_a = {}
mylist = []
audio_book = ["George R. R. Martin", "George R.R. Martin", "Yuval Harari", "Jon Stewart", "Stephen Colbert", "Mystery"]
...
if __name__ == "__main__":
    ...
    for key in glob.glob(MyPath + '*([3-99]).mp3'):
        print(key)
        only_files.remove(key[3:])
        try:
            only_files.remove(key[3:-8] + ".mp3")
            only_files.remove(key[3:-8] + " (1)" + ".mp3")
            only_files.remove(key[3:-8] + " (2)" + ".mp3")
        except:
            pass
    ...
    for f in only_files:
        ...
        for item in audio_book:
            ...
            if item in artist and isfile(join(MyPath, f)):
                print(artist)
